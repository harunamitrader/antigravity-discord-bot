...
3. ユーザーブロック以降のブロックをレスポンス対象として抽出
4. **ポジティブマッチング**: 各ブロック内で `leading-relaxed select-text` クラスを持つ要素のみを直接抽出。Thought・ログヘッダー（Ran command / Edited 等）・ボタン類はこの構造を持たないため、構造的に除外される。
5. 抽出した要素内の `<style>` と `<button>` を除去してテキスト取得
6. **回答全文テキスト**: 下から最大4000文字
7. **返信テキスト**: 下から最大1900文字

**技術メモ:**
- CDP式は `Function.toString()` + 文字列連結方式で生成（テンプレートリテラルのエスケープ問題を回避）
- テキストマッチングによる削除ロジックは廃止（v1.7.1〜）。DOM構造ベースのポジティブ抽出方式を採用。
...
### 3.6 ウィンドウ管理 (`/window`) とマルチタスク
- CDPで接続可能なAntigravity（VS Code）のウィンドウ一覧表示および接続先の切り替えを行います。
- **高度なマルチインスタンス対応 (Auto-Fallback)**:
  - `.env` の他に `.env.bot2` を配置しておくことで、同一ディレクトリから複数のBotプロセスを立ち上げることができます。
  - `src/env_loader.js` が起動時に `bot1.lock` を用いてOSレベルのプロセス死活監視（PIDチェック）を行います。
  - Primaryインスタンスが既に稼働中の場合、後発のプロセスは自動的に Secondary スロットに移行し、`.env.bot2`（2つ目のBotアカウント）を読み込んで起動します。
  - ユーザーは各Botに対し個別に `/window` コマンドを発行することで、1台のPC上で複数（2つ以上）のAntigravityウィンドウを独立して遠隔操作するマルチタスク環境を構築可能です。
