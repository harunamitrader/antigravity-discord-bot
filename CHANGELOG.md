...
- **CDPデバッグポートのデフォルトを `9222` → `9500` に変更** (`src/config.js`)
  - ポート9222はChromiumブラウザや各種開発ツールとの競合が発生しやすいため、より安全な `9500` をデフォルトに変更
  - Antigravityのショートカット起動引数も `--remote-debugging-port=9500` に更新が必要（既存インストール済みの場合は手動変更、または「おすすめの導入方法」のプロンプトで再作成を推奨）
  - この変更により、v1.5.1で追加された「Browser CDP Portを手動変更する手順」は**不要**になりました（詳細はREADMEの注記を参照）

### 変更されたファイル
| ファイル | 主な変更 |
|---|---|
| `src/config.js` | `PORTS` を `[9222]` から `[9500]` に変更 |
